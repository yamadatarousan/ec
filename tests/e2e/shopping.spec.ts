...
test.describe('ショッピング機能', () => {
  ...
  test('ウィッシュリスト機能', async ({ page }) => {
    ...
    // ウィッシュリスト追加成功
    await expect(page.locator('text=ウィッシュリストに追加しました')).toBeVisible()
    ...
    await expect(page.locator('text=ウィッシュリストから削除しました')).toBeVisible()
  })
  ...
  test('レスポンシブデザイン（モバイル）', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="add-to-cart-mobile"]')).toBeVisible()
  })
})